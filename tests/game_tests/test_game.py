...
@pytest.mark.parametrize('move_stack, repetition', [
    ...
    (deque([
        ...
        (Move(Color.WHITE, (BitBoard(0x00008000), BitBoard(0x00400000)), MoveType() | TAKE | Piece.PAWN,
              BitBoard(0x00040000)), 0),
        (Move(Color.BLACK, (BitBoard(0x04000000), BitBoard(0x00020000)), MoveType() | TAKE | Piece.PAWN,
              BitBoard(0x00400000)), 0),
    ]), False),
    (deque([
...
def test_three_fold_repetition(move_stack: deque[tuple[Move, int]], repetition: bool):
    ...
    Parameters
    ----------
    move_stack : deque[tuple[Move, int]]
    repetition : bool
    """
...
def test_push(board_init: tuple[BitBoard, BitBoard, BitBoard], moves: list[Move],
              board_fin: tuple[BitBoard, BitBoard, BitBoard], last_take: int):
    """
    Tests push
    Parameters
    ----------
    ...
    moves : list[Move]
    board_fin : tuple[BitBoard, BitBoard, BitBoard]
    last_take : int
    """
    board = Board()
...
@pytest.mark.parametrize('board_init, moves, board_fin, last_take', moves_and_ref_list)
def test_pop(board_init: tuple[BitBoard, BitBoard, BitBoard], moves: list[Move],
             board_fin: tuple[BitBoard, BitBoard, BitBoard], last_take: int):
    """
    Tests pop
    Parameters
...
@pytest.mark.parametrize('board_init, moves, board_fin, last_take', moves_and_ref_list)
def test_push_from_popped(board_init: tuple[BitBoard, BitBoard, BitBoard], moves: list[Move],
                          board_fin: tuple[BitBoard, BitBoard, BitBoard], last_take: int):
    """
    Tests push from popped
    Parameters
    ----------
    board_init : tuple[BitBoard, BitBoard, BitBoard]
    moves : list[Move]
...
@pytest.mark.parametrize('board_init, moves, board_fin, last_take', moves_and_ref_list)
def test_peek(board_init: tuple[BitBoard, BitBoard, BitBoard], moves: list[Move],
              board_fin: tuple[BitBoard, BitBoard, BitBoard], last_take: int):
    """
    Tests peek.
    Parameters
...
def test_get_moves(init_board_tuple: tuple[BitBoard, BitBoard, BitBoard], white_ref_set: set[Move],
...
def test_get_moves_from_mask(init_board: tuple[BitBoard, BitBoard, BitBoard], white_ref_positions: set[BitBoard],
                             black_ref_positions: set[BitBoard]):
    """
    Tests get moves from mask
    Parameters
    ----------
    init_board : tuple[BitBoard, BitBoard, BitBoard]
    white_ref_positions : set[BitBoard]
    black_ref_positions : set[BitBoard]
    """
    board = Board()
...
def test_get_moves_to_mask(init_board: tuple[BitBoard, BitBoard, BitBoard], turn: bool, from_mask: BitBoard,
                           ref_to_masks: set[BitBoard]):
    """
    Tests get moves to mask
    Parameters
    ----------
    init_board : tuple[BitBoard, BitBoard, BitBoard]
    turn : bool
...
def test_get_move_from_to(init_board: tuple[BitBoard, BitBoard, BitBoard], from_mask: BitBoard, to_mask: BitBoard,
                          ref_move: Move):
    """
    Tests get move from to
    Parameters
    ----------
    init_board : tuple[BitBoard, BitBoard, BitBoard]
    from_mask : BitBoard
...
@pytest.mark.parametrize('init_board, turn, move_stack, ref_res',
                         [
                             ((BitBoard(0x000000ff), BitBoard(0xff000000), BitBoard(0xff0000ff)), Color.WHITE,
                              deque([]), None),
                             ((BitBoard(0x000000ff), BitBoard(0x00000000), BitBoard(0x000000ff)), Color.WHITE,
                              deque([]), 1),
                             ...
                             (
                                     ...
                                     deque([
                                         (Move(Color.WHITE, (BitBoard(0x10000000), BitBoard(0x01000000)), MoveType(),
                                               BitBoard(0x00000000)), 30),
                                         (Move(Color.BLACK, (BitBoard(0x00000004), BitBoard(0x00000040)), MoveType(),
                                               BitBoard(0x00000000)), 31),
                                         (Move(Color.WHITE, (BitBoard(0x01000000), BitBoard(0x10000000)), MoveType(),
                                               BitBoard(0x00000000)), 32)
                                     ]),
                                     0
                             ),
                             (
                                     ...
                                     deque([
                                         (Move(Color.WHITE, (BitBoard(0x10000000), BitBoard(0x01000000)), MoveType(),
                                               BitBoard(0x00000000)), 20),
                                         (Move(Color.BLACK, (BitBoard(0x00000004), BitBoard(0x00000040)), MoveType(),
                                               BitBoard(0x00000000)), 21),
                                         (Move(Color.WHITE, (BitBoard(0x01000000), BitBoard(0x10000000)), MoveType(),
                                               BitBoard(0x00000000)), 22)
                                     ]),
                                     None
                             ),
                         ])
def test_get_result_train(init_board: tuple[BitBoard, BitBoard, BitBoard], turn: bool,
                          move_stack: deque[tuple[Move, int]], ref_res: int):
    """
    Tests get result train
    Parameters
    ----------
    init_board : tuple[BitBoard, BitBoard, BitBoard]
    turn : bool
...
def test_get_result(init_board: tuple[BitBoard, BitBoard, BitBoard], turn: bool, move_stack: deque[tuple[Move, int]],
                    ref_res: int):
    """
    Tests get result
    Parameters
...
def test_get_end_type(init_board: tuple[BitBoard, BitBoard, BitBoard], turn: bool, move_stack: deque[tuple[Move, int]],
                      game_end: GameEnd):
    """
    Tests get end type
    Parameters
...
@pytest.mark.parametrize('board_init, moves, board_fin, last_take', moves_and_ref_list)
def test_get_move_history(board_init: tuple[BitBoard, BitBoard, BitBoard], moves: list[Move],
                          board_fin: tuple[BitBoard, BitBoard, BitBoard], last_take: int):
    """
    Tests get move history
    Parameters
    ----------
    board_init : tuple[BitBoard, BitBoard, BitBoard]
    moves : list[Move]
