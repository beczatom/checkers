...
from app.game.board import Board, make_gen, is_color
from app.game.move import Move
from app.game.constants import BitBoard, Color, MoveType, PROMOTION, Piece, EVEN_ROW, TAKE


def dummy_fun_make_gen_test(bitboard: BitBoard) -> list[int]:
    """
    Dummy function for testing make_gen
    Parameters
    ----------
    bitboard : BitBoard
        used for dummy function

    Returns
    -------
    l : list[int]
        list that contains 1 or nothing
    """
...
def test_attacking_moves_in_direction():
    """
    Tests attacking moves in direction
    """
    board = Board()
    ...
    board.attacking_moves_in_direction(BitBoard(0x00000004), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000200)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000040))]
    moves = []

    board.attacking_moves_in_direction(BitBoard(0x00000002), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000002), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000040))]
    moves = []
    ...
    board.attacking_moves_in_direction(BitBoard(0x00000008), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000008), BitBoard(0x00000400)), MoveType() | TAKE | Piece.KING,
                          BitBoard(0x00000080))]
    moves = []
    ...
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000200)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000040)),
                     Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000800)), MoveType() | TAKE | Piece.KING,
                          BitBoard(0x00000080))]
    moves = []
    ...
    assert moves == [Move(Color.BLACK, (BitBoard(0x00080000), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00008000))]
    moves = []
    ...
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000080), BitBoard(0x00004000)), MoveType() | TAKE | Piece.KING,
                          BitBoard(0x00000400))]
    moves = []

    board.attacking_moves_in_direction(BitBoard(0x00000040), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000040), BitBoard(0x00008000)), MoveType() | TAKE | Piece.KING,
                          BitBoard(0x00000400))]
    moves = []

    board.attacking_moves_in_direction(BitBoard(0x00000020), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000020), BitBoard(0x00001000)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000100))]
    moves = []

    board.attacking_moves_in_direction(BitBoard(0x00000010), moves, board.black, True)
    assert moves == [Move(Color.WHITE, (BitBoard(0x00000010), BitBoard(0x00002000)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000100))]
    moves = []
    ...
    assert moves == [
        Move(Color.BLACK, (BitBoard(0x00000800), BitBoard(0x00000004)), MoveType() | PROMOTION | TAKE | Piece.PAWN,
             BitBoard(0x00000080))]
    moves = []

    board.attacking_moves_in_direction(BitBoard(0x00000400), moves, board.white, False)
    assert moves == [Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00000002)), MoveType() | TAKE | Piece.PAWN,
                          BitBoard(0x00000040)),
    ...
    assert moves == [
        Move(Color.BLACK, (BitBoard(0x00000100), BitBoard(0x00000002)), MoveType() | PROMOTION | TAKE | Piece.PAWN,
             BitBoard(0x00000020))]


def test_is_valid_not_attacking_move():
    """
    Tests is valid not attacking move
    """
    board = Board()
...
def test_validate_not_attacking_move():
    """
    Tests validate not attacking move
    """
    board = Board()
...
def test_not_attacking_moves_in_direction():
    """
    Tests not attacking moves in direction
    """
    board = Board()
...
@pytest.mark.parametrize('init_board_tuple, move, ref_board_tuple', [
    ...
    (
            (BitBoard(0x040040d0), BitBoard(0x0000880a), BitBoard(0x0400c8d0)),
            Move(Color.BLACK, (BitBoard(0x00000008), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
                 BitBoard(0x00000080)),
            (BitBoard(0x04004050), BitBoard(0x00008c02), BitBoard(0x0400c850))
    ),
    (
            (BitBoard(0x04004050), BitBoard(0x00008c02), BitBoard(0x0400c850)),
            Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00020000)), MoveType() | TAKE | Piece.PAWN,
                 BitBoard(0x00004000)),
    ...
    (
            ...
            Move(Color.WHITE, (BitBoard(0x00800000), BitBoard(0x40000000)), MoveType() | PROMOTION | TAKE | Piece.PAWN,
                 BitBoard(0x04000000)),
            (BitBoard(0x40000000), BitBoard(0x02000000), BitBoard(0x02000000))
    ),
])
def test_make_move_changes(init_board_tuple: tuple[BitBoard, BitBoard, BitBoard], move: Move,
                           ref_board_tuple: tuple[BitBoard, BitBoard, BitBoard]):
    """
    Tests make move changes
    Parameters
    ----------
    init_board_tuple : tuple[BitBoard, BitBoard, BitBoard]
    move : Move
    ref_board_tuple : tuple[BitBoard, BitBoard, BitBoard]
    """
    board = Board()
...
@pytest.mark.parametrize('init_board_tuple, white_ref_set, black_ref_set', [
    ...
    (
            (BitBoard(0x000840f4), BitBoard(0x00808d00), BitBoard(0x0088c9f4)),
            {
                Move(Color.WHITE, (BitBoard(0x00080000), BitBoard(0x04000000)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00800000)),
                Move(Color.WHITE, (BitBoard(0x00000020), BitBoard(0x00001000)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00000100)),
                Move(Color.WHITE, (BitBoard(0x00000010), BitBoard(0x00002000)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00000100))
            },
            {
                Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00020000)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00004000)),
                Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00000002)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00000040)),
                Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00000008)), MoveType() | TAKE | Piece.PAWN,
                     BitBoard(0x00000080)),
...
def test_get_legal_moves(init_board_tuple: tuple[BitBoard, BitBoard, BitBoard], white_ref_set: set[Move],
                         black_ref_set: set[Move]):
    """
    Tests get legal moves
    Parameters
    ----------
    init_board_tuple : tuple[BitBoard, BitBoard, BitBoard]
    white_ref_set : set[Move]
    black_ref_set : set[Move]
    """
    board = Board()
    board.white = init_board_tuple[0]
...
def test_not_attacking_moves_from_pos():
    """
    Tests not attacking moves from pos
    """
    board = Board()
    board.white = BitBoard(0x0000000f)
    board.black = BitBoard(0x00000000)
...
def test_attacking_moves_from_pos():
    """
    Tests attacking moves from pos
    """
    board = Board()
    board.white = BitBoard(0x0000000f)
    board.black = BitBoard(0x00000000)
    ...
    assert board.attacking_moves_from_pos(BitBoard(0x00000008)) == []
    assert board.attacking_moves_from_pos(BitBoard(0x00000004)) == [
        Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000200)), MoveType() | TAKE | Piece.PAWN,
             BitBoard(0x00000040))]
    assert board.attacking_moves_from_pos(BitBoard(0x00000002)) == [
        Move(Color.WHITE, (BitBoard(0x00000002), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
             BitBoard(0x00000040))]
    assert board.attacking_moves_from_pos(BitBoard(0x00000001)) == []
    ...
    assert board.attacking_moves_from_pos(BitBoard(0x00000008)) == [
        Move(Color.WHITE, (BitBoard(0x00000008), BitBoard(0x00000400)), MoveType() | TAKE | Piece.KING,
             BitBoard(0x00000080))]
    assert board.attacking_moves_from_pos(BitBoard(0x00000004)) == [
        Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000200)), MoveType() | TAKE | Piece.PAWN,
             BitBoard(0x00000040)),
    ...
    assert board.attacking_moves_from_pos(BitBoard(0x00080000)) == [
        Move(Color.BLACK, (BitBoard(0x00080000), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
             BitBoard(0x00008000))]
    ...
    assert board.attacking_moves_from_pos(BitBoard(0x00000080)) == [
        Move(Color.WHITE, (BitBoard(0x00000080), BitBoard(0x00004000)), MoveType() | TAKE | Piece.KING,
             BitBoard(0x00000400))]
    assert board.attacking_moves_from_pos(BitBoard(0x00000040)) == [
        Move(Color.WHITE, (BitBoard(0x00000040), BitBoard(0x00008000)), MoveType() | TAKE | Piece.KING,
             BitBoard(0x00000400))]
...
def test_make_move():
    ...
    board.make_move(Move(Color.WHITE, (BitBoard(0x00080000), BitBoard(0x04000000)), MoveType() | TAKE | Piece.PAWN,
                         BitBoard(0x00800000)))
    ...
    board.make_move(Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00000008)), MoveType() | TAKE | Piece.PAWN,
                         BitBoard(0x00000080)))
    ...
    board.make_move(
        Move(Color.WHITE, (BitBoard(0x00000004), BitBoard(0x00000080)), MoveType(0x00), BitBoard(0x00000000)))
    ...
    board.make_move(Move(Color.BLACK, (BitBoard(0x00000008), BitBoard(0x00000400)), MoveType() | TAKE | Piece.PAWN,
                         BitBoard(0x00000080)))
    ...
    board.make_move(Move(Color.BLACK, (BitBoard(0x00000400), BitBoard(0x00020000)), MoveType() | TAKE | Piece.PAWN,
                         BitBoard(0x00004000)))
...
def test_undo_move():
    ...
    board.undo_move(Move(Color.BLACK, (BitBoard(0x00080000), BitBoard(0x04000000)), MoveType() | TAKE | Piece.PAWN,
                         BitBoard(0x00800000)))
